...
void GameButton::commonLoad(std::string filenamePressed, std::string filenameReleased, int x, int y)
{
	buttonNotPressedTexture.loadFromFile(filenameReleased);
	buttonPressedTexture.loadFromFile(filenamePressed);
	buttonSprite.setTexture(buttonNotPressedTexture);
	buttonSprite.setOrigin(buttonNotPressedTexture.getSize().x / 2, buttonNotPressedTexture.getSize().y / 2);
	buttonSprite.setPosition(x, y);
}
